class CRC32:
    ...
    Based on the code provided by the lecturers of the course.
    """
    ...
    def memcrc(b):
        ...
        for ch in b:
            tabidx = (s >> 24) ^ ch
            s = CRC32.UNSIGNED((s << 8)) ^ CRC32.crctab[tabidx]

        while n:
            c = n & 0o377
            n = n >> 8
            s = CRC32.UNSIGNED(s << 8) ^ CRC32.crctab[(s >> 24) ^ c]
        return CRC32.UNSIGNED(~s)

    def file_crc_calc(fname):
        try:
            buffer = open(fname, 'rb').read()
            return CRC32.memcrc(buffer)
        except IOError:
