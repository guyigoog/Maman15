...
void CRC32::update(char* buf, size_t buf_size)
{
    auto size = (uint32_t)buf_size;
    uint32_t crc = this->crc;
    for (uint32_t i = 0; i < size; i++)
    {
        crc = crctab[(crc >> 24) ^ buf[i]] ^ ((crc << 8) & 0xffffffff);
    }
    this->crc = crc;
    nchars += size;
}
...
uint32_t CRC32::fileCRCcalc(std::string file_path)
{
    unsigned int  BUF_SIZE = 4098;
    char* buf = new char[BUF_SIZE];
    std::ifstream file(file_path, std::ios::binary);

    if (!file.is_open())
    {
        std::string err = "Cannot open " + file_path;
        throw std::exception(err.c_str());
    }
    size_t chars;
    while (!file.eof()) {
        chars = file.read(buf, BUF_SIZE).gcount();
        update(buf, chars);
    }
    delete[] buf;
    return  digest();
